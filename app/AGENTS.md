...
- **Gradle**: Wrapper is committed; always invoke `./gradlew`. AGP 9 / Gradle 9 (see root `AGENTS.md` for cross-module
  version alignment rules with the `Core` and `multiplatform-calendar` submodules).
- **Kotlin plugin**: AGP 9's built-in Kotlin support is used — do **not** apply `core.plugins.kotlin.android` /
  `org.jetbrains.kotlin.android` in any Android module's `plugins {}` block (applying it fails the build since it is no
  longer required, see https://kotl.in/gradle/agp-built-in-kotlin).
...
- **Debug signing**: Apply `core.plugins.infomaniak.android.debug.signing` immediately after
  `core.plugins.android.application`. It configures only the existing debug signing configuration and materializes Core's
  shared public development keystore under the git-ignored `.gradle/infomaniak/` directory.
