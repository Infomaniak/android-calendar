...
| `:CalendarComponents:EventDetail` | `com.infomaniak.calendar.components.eventdetail` | Event detail module — the `detail` package contains the `EventDetail` Composable for a single event's full detail (title, date and time, …), driven by the `EventDetailUi` model, while the `edit` package currently exposes a placeholder `EventEdit` Composable. Re-exports Foundation and `core.infomaniak.core.filetypes` via `api` since both appear in `EventDetailUi`. |
