...
│   ├── EventDetail/                # Event detail module (`detail` UI + placeholder `edit` UI)
...
| `:CalendarComponents:EventDetail` | `com.infomaniak.calendar.components.eventdetail` | Event detail module — the `detail` package contains the `EventDetail` Composable for a single event's full detail (date and time, attendees, …), driven by the `EventDetailUi` model, next to `EventDetailTitle`, the event's color and title kept apart with no text style of its own so the consumer can host it in its own collapsing top app bar and have the title shrink into the toolbar while the detail scrolls; the `edit` package currently exposes a placeholder `EventEdit` Composable. Re-exports Foundation and `core.infomaniak.core.filetypes` via `api` since both appear in `EventDetailUi`. |
...
| `EventEdit` (EventDetail)             | 🚧 Placeholder                |
