...
private fun Event(
    ...
    Row(
        ...
        when (event) {
            is EventUi.Normal -> EventItem(event, onClick = { goToEventDetail(event.id) }, Modifier.fillMaxWidth())
...
private fun PreviewPlanning(@PreviewParameter(WeekEventsPreviewParameter::class) weekEvents: Map<YearWeek, Map<LocalDate, List<EventUi>>>) {
    Surface {
        Planning(goToEventCreation = {}, goToEventDetail = {}, weekEvents = { weekEvents })
    }
}
