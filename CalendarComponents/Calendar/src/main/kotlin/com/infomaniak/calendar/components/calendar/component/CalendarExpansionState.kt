...
class CalendarExpansionState internal constructor(initiallyExpanded: Boolean, private val coroutineScope: CoroutineScope) {
    ...
    /**
     * Folds the calendar into the scroll of the content below it, the way Google Calendar does.
     *
     * Collapsing happens in [onPreScroll]: swiping up moves the calendar out of the way before the content
     * starts scrolling, because the calendar is what the finger is pushing away. Expanding happens in
     * [onPostScroll] instead, so it may only use the scroll the content had no room left to take, which is
     * exactly the over-scroll at the top of the list.
     *
     * Both are limited to [NestedScrollSource.UserInput]: the expansion follows a finger, never a fling
     * running on after it, and never a programmatic scroll such as jumping to a tapped day.
     */
